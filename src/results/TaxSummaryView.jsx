...
export default function TaxSummaryView({ data, results, reset, editStep }) {
  ...
  return (
    <div className="min-h-screen bg-slate-950 pb-24 font-sans text-white">
      ...
      {/* Header */}
      <header className="bg-slate-900/80 backdrop-blur-md border-b border-slate-700 px-6 py-4 flex justify-between items-center sticky top-0 z-30">
        <div className="flex items-center gap-2.5">
          ...
          <span className="text-lg font-black text-white tracking-tight">Shivansh's tax calculator app</span>
        </div>
        <button onClick={reset} className="text-sm font-bold text-slate-500 hover:text-slate-900 transition-colors flex items-center gap-1.5 bg-slate-800 hover:bg-slate-200 px-3 py-1.5 rounded-full">
      ...
      <main className="max-w-6xl mx-auto px-4 mt-8 space-y-8">
        ...
        <section className={`rounded-[2rem] ${heroTheme.bg} p-8 sm:p-12 text-white shadow-2xl shadow-emerald-900/20 overflow-hidden relative`}>
          ...
          <div className="relative z-10">
            <div className="inline-flex items-center gap-2 bg-slate-800/50 backdrop-blur-md px-4 py-1.5 rounded-full text-xs font-bold uppercase tracking-widest mb-6 border border-white/20 shadow-md shadow-black/20">
            ...
            <div className="flex flex-wrap lg:flex-nowrap gap-6 lg:gap-0 lg:divide-x lg:divide-white/20 bg-slate-800/50 backdrop-blur-xl border border-white/20 p-6 rounded-3xl shadow-2xl">
        ...
        <section className="bg-slate-900 rounded-[2rem] border border-slate-700 p-8 flex flex-col lg:flex-row items-center gap-8 shadow-md shadow-black/20">
          <div className="flex items-center gap-4 lg:w-1/4 shrink-0">
            <div className="w-12 h-12 rounded-2xl bg-emerald-900/30 flex items-center justify-center text-emerald-500 border border-emerald-800">
              <svg className="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M9.663 17h4.673M12 3v1m6.364 1.636l-.707.707M21 12h-1M4 12H3m3.343-5.657l-.707-.707m2.828 9.9a5 5 0 117.072 0l-.548.547A3.374 3.374 0 0014 18.469V19a2 2 0 11-4 0v-.531c0-.895-.356-1.754-.988-2.386l-.548-.547z" /></svg>
            </div>
            <h2 className="text-xl font-black text-white leading-tight">Why {isRefund ? "you're getting a refund" : isPayable ? "you have tax payable" : "your tax is settled"}</h2>
          </div>
          <div className="flex-1 grid grid-cols-1 md:grid-cols-3 gap-6 lg:gap-8 border-t lg:border-t-0 lg:border-l border-slate-800 pt-6 lg:pt-0 lg:pl-10">
            <div className="flex items-start gap-3">
              <CheckCircle className="w-6 h-6 text-emerald-500 shrink-0" />
              <p className="text-slate-500 font-medium">Your total tax liability is computed at <strong className="text-white">{fmt(primaryRegime.totalTax)}</strong>.</p>
            </div>
            <div className="flex items-start gap-3">
              <CheckCircle className="w-6 h-6 text-emerald-500 shrink-0" />
              <p className="text-slate-500 font-medium">You've already paid <strong className="text-white">{fmt(results.tdsDeducted)}</strong> as TDS.</p>
            </div>
            <div className="flex items-start gap-3">
              <CheckCircle className="w-6 h-6 text-emerald-500 shrink-0" />
              <p className="text-slate-500 font-medium">The difference is the final amount to be <strong className="text-white">{isRefund ? 'refunded' : isPayable ? 'paid' : 'settled'}</strong>.</p>
            </div>
          </div>
        </section>
        ...
        <div className="pt-6">
          <h2 className="text-3xl font-black text-white mb-8 tracking-tight">Regime Comparison</h2>
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-stretch">
            
            {/* PRIMARY CARD (Left, ~45% width) - Premium Glow */}
            <div className="lg:col-span-5 bg-slate-900 rounded-[2rem] border-2 border-emerald-400 shadow-[0_8px_30px_rgb(52,211,153,0.15)] overflow-hidden relative flex flex-col">
              <div className="absolute top-0 right-0 bg-gradient-to-r from-emerald-500 to-emerald-600 text-white text-[10px] font-bold uppercase tracking-wider px-4 py-2 rounded-bl-xl shadow-md shadow-black/20">
                Recommended
              </div>
              <div className="p-8 md:p-10 flex-1 flex flex-col">
                <div className="flex items-center gap-4 mb-10">
                  <div className="w-14 h-14 rounded-full bg-emerald-900/30 flex items-center justify-center text-emerald-600 border border-emerald-800 shadow-inner">
                  ...
                  <div>
                    <h3 className="text-2xl font-black text-white leading-tight">{primaryName}</h3>
                ...
                <div className="grid grid-cols-2 gap-6 mb-10">
                  <div>
                    ...
                    <div className="text-4xl font-black text-white tracking-tight">{fmt(primaryRegime.totalTax)}</div>
                  </div>
                  <div>
                ...
                <div className="space-y-4 mb-10 flex-1">
                  <div className="flex items-center gap-3">
                    ...
                    <span className="text-base text-slate-300 font-semibold">Lower overall tax liability</span>
                  </div>
                  <div className="flex items-center gap-3">
                    <CheckCircle className="w-5 h-5 text-emerald-500 shrink-0" />
                    <span className="text-base text-slate-300 font-semibold">{recommended === 'new' ? 'No need for investments or deductions' : 'Maximizes your declared deductions'}</span>
                  </div>
                </div>
            ...
            <div className="lg:col-span-4 bg-slate-900 rounded-[2rem] border border-slate-700 p-8 flex flex-col shadow-md shadow-black/20">
              <div className="flex justify-between items-start mb-8">
                <div className="flex items-center gap-4">
                  <div className="w-12 h-12 rounded-full bg-rose-900/30 flex items-center justify-center text-rose-500 border border-rose-800">
                  ...
                  <div>
                    <h3 className="text-xl font-bold text-white leading-tight">{secondaryName}</h3>
              ...
              <div className="text-[11px] font-bold text-slate-500 uppercase tracking-widest mb-4">Deductions Considered</div>
              <div className="space-y-4 mb-8">
                <div className="flex justify-between border-b border-slate-800 pb-3">
                  <span className="text-sm text-slate-500 font-medium">Standard Deduction</span>
                  <span className="text-sm font-bold text-white">{fmt(secondaryRegime.standardDeduction)}</span>
                </div>
                {secondaryRegime.professionalTaxDeduction > 0 && (
                  <div className="flex justify-between border-b border-slate-800 pb-3">
                    <span className="text-sm text-slate-500 font-medium">Professional Tax</span>
                    <span className="text-sm font-bold text-white">{fmt(secondaryRegime.professionalTaxDeduction)}</span>
                  </div>
                )}
                {secondaryRegime.employerNPSDeduction > 0 && (
                  <div className="flex justify-between border-b border-slate-800 pb-3">
                    <span className="text-sm text-slate-500 font-medium">Employer NPS</span>
                    <span className="text-sm font-bold text-white">{fmt(secondaryRegime.employerNPSDeduction)}</span>
                  </div>
                )}
                {secondaryRegime.deduction80C > 0 && (
                  <div className="flex justify-between border-b border-slate-800 pb-3">
                    <span className="text-sm text-slate-500 font-medium">80C Investments</span>
                    <span className="text-sm font-bold text-white">{fmt(secondaryRegime.deduction80C)}</span>
                  </div>
                )}
                {secondaryRegime.deduction80D > 0 && (
                  <div className="flex justify-between border-b border-slate-800 pb-3">
                    <span className="text-sm text-slate-500 font-medium">80D Medical</span>
                    <span className="text-sm font-bold text-white">{fmt(secondaryRegime.deduction80D)}</span>
                  </div>
                )}
              </div>
              ...
              <div className="mt-auto">
                {savings > 0 ? (
                  <div className="bg-rose-900/30 text-rose-700 text-sm font-bold py-4 px-4 rounded-xl flex items-center justify-center gap-2 border border-rose-800">
                  ...
                  <div className="bg-slate-950 text-slate-500 text-sm font-bold py-4 px-4 rounded-xl flex items-center justify-center border border-slate-700">
            ...
            <div className="lg:col-span-3 bg-slate-900 rounded-[2rem] border border-slate-700 p-8 flex flex-col shadow-md shadow-black/20">
              <h3 className="text-sm font-bold text-white mb-8 uppercase tracking-widest">Tax Comparison</h3>
              
              <div className="space-y-8 flex-1">
                <div>
                  <div className="flex justify-between items-end mb-3">
                    <span className="text-xs font-bold text-slate-500">{primaryName}</span>
                ...
                <div>
                  <div className="flex justify-between items-end mb-3">
                    <span className="text-xs font-bold text-slate-500">{secondaryName}</span>
                    <span className="text-base font-black text-white">{fmt(secondaryRegime.totalTax)}</span>
                  </div>
                  <div className="w-full bg-slate-800 rounded-full h-3 overflow-hidden shadow-inner">
                    <div 
              ...
              {savings > 0 && (
                <div className="mt-10 bg-emerald-900/30 rounded-2xl p-6 text-center border border-emerald-200 shadow-[inset_0_2px_10px_rgb(52,211,153,0.1)]">
        ...
        <div className="bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 border border-slate-700 rounded-[2rem] p-8 md:p-10 mt-8 flex flex-col md:flex-row items-center justify-between gap-8 shadow-2xl shadow-black/50">
          <div className="flex items-center gap-5 text-left w-full md:w-auto">
            <div className="w-16 h-16 rounded-2xl bg-slate-800/50 flex items-center justify-center text-white shrink-0 shadow-md shadow-black/20 border border-white/20 backdrop-blur-md">
          ...
          <div className="flex flex-col sm:flex-row items-center gap-4 w-full md:w-auto">
            <button className="w-full sm:w-auto bg-slate-900 hover:bg-slate-50 text-white font-black py-4 px-8 rounded-xl transition-all shadow-lg flex items-center justify-center gap-2 group">
            ...
            <button className="w-full sm:w-auto bg-slate-800 border border-slate-600 hover:bg-slate-700 text-white font-bold py-4 px-6 rounded-xl transition-colors shadow-md shadow-black/20 flex items-center justify-center gap-2">
        ...
        {/* 6. TRUST SIGNALS */}
        <div className="border-t border-slate-700 mt-12 pt-8 flex flex-col md:flex-row justify-center items-start md:items-center gap-8 md:gap-16 pb-12 px-4">
          <div className="flex items-center gap-4">
            <div className="w-12 h-12 rounded-2xl bg-emerald-900/30 flex items-center justify-center text-emerald-600 shrink-0 border border-emerald-800">
               <Shield className="w-6 h-6" />
            </div>
            <div>
              <div className="text-sm font-black text-white leading-tight">Secure & compliant</div>
          ...
          <div className="flex items-center gap-4">
            <div className="w-12 h-12 rounded-2xl bg-blue-900/30 flex items-center justify-center text-blue-600 shrink-0 border border-blue-800">
               <svg className="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z" /></svg>
            </div>
            <div>
              <div className="text-sm font-black text-white leading-tight">Accurate calculations</div>
          ...
          <div className="flex items-center gap-4">
            <div className="w-12 h-12 rounded-2xl bg-violet-900/30 flex items-center justify-center text-violet-600 shrink-0 border border-violet-800">
            ...
            <div>
              <div className="text-sm font-black text-white leading-tight">Expert support</div>
