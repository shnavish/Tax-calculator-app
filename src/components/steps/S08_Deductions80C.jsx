...
export default function S08_Deductions80C(props) {
  ...
  return (
    <StepWrapper {...props} stepName="80C Deductions">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-6">
        {OPTIONS_80C.map(opt => {
          ...
          return (
            <div 
              key={opt.id}
              className={`rounded-xl border-2 transition-all overflow-hidden ${isSelected ? 'border-indigo-600 bg-indigo-50/30' : 'border-gray-200 bg-white hover:border-gray-300'}`}
            >
              <div 
              ...
              >
                ...
                <div>
                  <div className={`text-sm font-bold ${isSelected ? 'text-indigo-900' : 'text-gray-900'}`}>{opt.label}</div>
                  <div className="text-xs text-gray-500">{opt.desc}</div>
                </div>
              </div>

              {isSelected && (
                <div className="px-4 pb-4 pt-1 reveal border-t border-indigo-100 bg-indigo-50/50">
                  <NumberInput
                    id={opt.id}
                    label="Annual Amount"
      ...
      {total80C > 0 && (
        <div className={`mb-8 px-4 py-3 rounded-xl border flex items-center justify-between ${isCapped ? 'bg-green-50 border-green-200' : 'bg-indigo-50 border-indigo-200'}`}>
          <div>
            <div className={`text-xs font-bold uppercase tracking-wide ${isCapped ? 'text-green-700' : 'text-indigo-700'}`}>
              Total 80C Declared
            </div>
            {isCapped && <div className="text-[10px] text-green-600 mt-0.5">Max cap of ₹1.5L reached!</div>}
          </div>
          <div className={`text-xl font-black ${isCapped ? 'text-green-700' : 'text-indigo-700'}`}>
