  ...
  return (
    <StepWrapper {...props} stepName="HRA & Rent">
      <div className="mb-4 flex items-center gap-2">
        ...
        <div className="text-xs font-medium text-indigo-600 uppercase tracking-wide">Exemptions</div>
      </div>
        ...
        <div>
          <div className="flex gap-3 mb-3">
            <button
              ...
              className={`flex-1 py-2.5 px-4 rounded-xl border-2 font-semibold transition-colors ${data.paysRent === true ? 'border-indigo-600 bg-indigo-50 text-indigo-700' : 'border-gray-200 bg-white text-gray-700 hover:border-gray-300 hover:bg-gray-50'}`}
            >
              Yes
            </button>
            <button
              onClick={() => { update({ paysRent: false, monthlyRent: '', cityType: null }); setErrors(e => ({...e, paysRent: null, monthlyRent: null, cityType: null})) }}
              className={`flex-1 py-2.5 px-4 rounded-xl border-2 font-semibold transition-colors ${data.paysRent === false ? 'border-indigo-600 bg-indigo-50 text-indigo-700' : 'border-gray-200 bg-white text-gray-700 hover:border-gray-300 hover:bg-gray-50'}`}
            >
              No
            </button>
          </div>
          ...
          {data.paysRent && (
            <div className="bg-indigo-50/50 border border-indigo-100 rounded-xl p-4 reveal space-y-5 mt-4">
              <div>
                <NumberInput
                  id="rent"
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">City type <span className="text-red-500">*</span></label>
                <div className="flex gap-2">
                  <button
                    onClick={() => { update({ cityType: 'metro' }); setErrors(e => ({...e, cityType: null})) }}
                    className={`flex-1 py-2 px-3 rounded-lg border text-sm font-semibold transition-colors ${data.cityType === 'metro' ? 'border-indigo-600 bg-indigo-600 text-white' : 'border-gray-300 bg-white text-gray-700 hover:bg-gray-50'}`}
                  >
                    Metro (50%)
                  </button>
                  <button
                    onClick={() => { update({ cityType: 'nonMetro' }); setErrors(e => ({...e, cityType: null})) }}
                    className={`flex-1 py-2 px-3 rounded-lg border text-sm font-semibold transition-colors ${data.cityType === 'nonMetro' ? 'border-indigo-600 bg-indigo-600 text-white' : 'border-gray-300 bg-white text-gray-700 hover:bg-gray-50'}`}
                  >
                    Non-Metro (40%)
                  </button>
                </div>
        ...
        <div className="pt-6 border-t border-gray-100">
          ...
          <div className="flex gap-3 mb-3">
            <button
              ...
              className={`flex-1 py-2.5 px-4 rounded-xl border-2 font-semibold transition-colors ${data.hasHRA === true ? 'border-indigo-600 bg-indigo-50 text-indigo-700' : 'border-gray-200 bg-white text-gray-700 hover:border-gray-300 hover:bg-gray-50'}`}
            >
              Yes
            </button>
            <button
              onClick={() => { update({ hasHRA: false, hraMonthly: '' }); setErrors(e => ({...e, hasHRA: null, hraMonthly: null})) }}
              className={`flex-1 py-2.5 px-4 rounded-xl border-2 font-semibold transition-colors ${data.hasHRA === false ? 'border-indigo-600 bg-indigo-50 text-indigo-700' : 'border-gray-200 bg-white text-gray-700 hover:border-gray-300 hover:bg-gray-50'}`}
            >
              No
            </button>
          </div>
