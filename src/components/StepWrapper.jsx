...
export default function StepWrapper({ children, goBack, reset, showProgress, progressStep, TOTAL_PROGRESS, step, data, stepName }) {
  return (
    <div className="min-h-screen bg-slate-950 flex flex-col">
      <div className="sticky top-0 z-10 bg-slate-900/95 backdrop-blur border-b border-slate-800 shadow-md shadow-black/20">
        <div className="max-w-7xl mx-auto px-4 sm:px-8 py-2.5 flex items-center gap-3">
          <button onClick={reset} className="flex items-center gap-2 text-left shrink-0">
            ...
            <div>
              <div className="text-sm font-bold text-white tracking-tight leading-none">Shivansh's tax calculator app</div>
            </div>
          </button>

          {step > 1 && (
            <button onClick={goBack} className="flex items-center gap-1 text-xs font-medium text-slate-500 bg-slate-800 hover:bg-slate-700 rounded-full px-3 py-1.5 transition-colors shrink-0">
          ...
          <div className="hidden md:flex items-center gap-1.5 shrink-0 ml-auto">
            ...
            <div>
              <div className="text-[10px] font-semibold text-slate-500 leading-none">100% Private</div>
              <div className="text-[10px] text-slate-500 leading-none mt-0.5">Data stays in your browser</div>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="flex-1 max-w-7xl mx-auto w-full px-4 sm:px-8 py-6">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 lg:gap-8">
          <main className="lg:col-span-7">
            <div className="bg-slate-900 rounded-2xl border border-slate-800 shadow-md shadow-black/20 p-5 sm:p-7">
              {children}
            </div>
          </main>
          {data && (
